...
def getPhotoById():
    try:
        if session.get('user'):
            ...
            photo = []
            photo.append({'Id':result[0][0],'Geo_Location':result[0][1],'Capture_Date':result[0][2],'Capture_By':result[0][3],'Album':result[0][4],'photo_link':result[0][5],'Shareable':result[0][6],'User':result[0][7]})
...
def addPhoto():
    try:
        if session.get('user'):
            p_Geo_Location = request.form['Geo_Location']
            p_Capture_Date = request.form['Capture_Date']
            p_Capture_By = session.get('Capture_By')
            p_Album = session.get('Album')
            p_Photo_Link = session.get('Photo_Link')
            p_Shareable = session.get('Shareable')
            p_user = session.get('User')
            conn = mysql.connect()
            cursor = conn.cursor()
            cursor.callproc('sp_addPhoto',(p_Geo_Location,p_Capture_Date,p_Capture_By,p_Album,p_Photo_Link,p_Shareable,p_user))
            data = cursor.fetchall()
